 ...
 */
package backpropagation.sigmoid;
...
public class NeuralNetwork {
    ...
    private static double[][] readData() throws Exception {
        File f = new File("./backpropagation/sigmoid/dados.copel.txt");
        FileReader fr = new FileReader(f);
        BufferedReader br = new BufferedReader(fr);
        
        String line;
        
        // ler metadado número de registros
        line=br.readLine();
        StringTokenizer st = new StringTokenizer(line);
        st.nextToken(" ");
        int registros = (new Integer(st.nextToken(" ")));

        // ler metadado n de colunas
        line=br.readLine();
        st = new StringTokenizer(line);
        st.nextToken(" ");
        int colunas = (new Integer(st.nextToken(" ")));
        
        
        double dados[][] = new double[registros][colunas];
        String tmp;
    ...
    //            write(st.countTokens());
                ...
                for (int c=0; c<colunas; c++){
                    tmp = st.nextToken("\t");
                    tmp = tmp.replaceAll(",", ".");
                    dados[r][c] = new Double(tmp);
                    write(dados[r][c]+ "\t");
                }
            } else {
                write("Desconsiderando linha em branco");
            }
            write("\n");
        }
        return dados;
    ...
    public static void main(String[] args) throws Exception {
        File f = new File(".");
        writeln("Diretório de trabalho: "+f.getCanonicalPath());
        // Inputs 
        double data[][] = readData();
        int nRegisters = 100;//data.length;
        int nInputs = data[0].length-1;
        double inputs[][] = new double[nRegisters][nInputs]; 
        for (int r=0;r<nRegisters; r++){
            for (int c=0; c<nInputs; c++){
                inputs[r][c]=data[r][c];
               write(inputs[r][c]+" inputs["+r+"]["+c+"] "); 
            }
            writeln(" ");
        }

        // Corresponding outputs, copel training data
        double expectedOutputs[][] = new double[nRegisters][1];
        for (int r=0;r<nRegisters; r++){
            expectedOutputs[r][0]=data[r][6];
               write(expectedOutputs[r][0]+" inputs["+r+"]["+0+"] "); 
        }
        

        double resultOutputs[][] = new double[nRegisters][1]; // dummy init
        for (int r=0;r<nRegisters; r++){
            resultOutputs[r][0]= -1;
        }
        ...
        // Inicializar a rede
        NeuralNetwork nn = new NeuralNetwork(nInputs, 4, 1);
        nn.setInputs(inputs);
        nn.setExpectedOutputs(expectedOutputs);
        nn.setResultOutputs(resultOutputs);
        int maxRuns = 50000;
        ...
        // Executar o teste em um dado de entrada
        int nTest = 118;
        double inputTest[] = new double[inputs[0].length];
        for (int i=0;i<inputTest.length;i++){
            inputTest[i]=data[nTest][i];
        }
        nn.setInput(inputTest);
        nn.activate();
        ...
        write("Valor esperado: ");
        writeln(data[nTest][6]+ " ");
               
        write("Dado resultante da rede: ");
    ...
    public void setExpectedOutputs(double eo[][]) {
        expectedOutputs = eo;
    }
    
    public void setResultOutputs(double ro[][]) {
        resultOutputs = ro;
    }
